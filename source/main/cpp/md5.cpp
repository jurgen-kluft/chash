...
#include "CString\StringFormatter.h"
...
/**
@brief Check if one hash value is smaller than antoher hash value
...
**/
bool					MD5Hash::operator<(rcMD5Hash inRHS) const
{
	for (int i=0; i<16; i++)
	{
		uint8 lhs=mData8[i], rhs=inRHS.mData8[i];
...
/**
@brief Check if one hash value is larger than antoher hash value 
...
void					MD5Hash::SetMD5(uint32 inR1, uint32 inR2, uint32 inR3, uint32 inR4)
{
	mData32[0] = inR1;
	mData32[1] = inR2;
	mData32[2] = inR3;
...
void					MD5Hash::GetMD5(rUInt32 outR1, rUInt32 outR2, rUInt32 outR3, rUInt32 outR4) const
{
	outR1 = mData32[0];
	outR2 = mData32[1];
	outR3 = mData32[2];
...
**/
bool					MD5Hash::ToString(rString outString) const
{
	outString = StringFormatter::FormatString("%02x%02x%02x%02",  xva_list(xva(mData8[0]),  xva(mData8[1]),  xva( mData8[2]), xva(mData8[3])));
	outString += StringFormatter::FormatString("%02x%02x%02x%02", xva_list(xva(mData8[4]),  xva(mData8[5]),  xva( mData8[6]), xva(mData8[7])));
	outString += StringFormatter::FormatString("%02x%02x%02x%02", xva_list(xva(mData8[8]),  xva(mData8[9]),  xva(mData8[10]), xva(mData8[11])));
	outString += StringFormatter::FormatString("%02x%02x%02x%02", xva_list(xva(mData8[12]), xva(mData8[13]), xva(mData8[14]), xva(mData8[15])));
...
**/
bool					MD5Hash::FromString(rcString inString)
{
	uint32 d[16];
	if (gStringScan(inString, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", xvr_list(xvr(&d[0]), xvr(&d[1]), xvr(&d[2]), xvr(&d[3]), xvr(&d[4]), xvr(&d[5]), xvr(&d[6]), xvr(&d[7]), xvr(&d[8]), xvr(&d[9]), xvr(&d[10]), xvr(&d[11]), xvr(&d[12]), xvr(&d[13]), xvr(&d[14]), xvr(&d[15]))) == 16)
	{
		for (int i=0; i<16; i++)
			mData8[i] = d[i];
		return true;
...
@brief		Byteswap an area of memory
			...
			This area converts a number of uint32's to little endian format to be compatible
			with the MD5 hash algorithm.

@param		ioBuffer	Pointer to data to swap
@param		inLength	Number of uint32's to swap
**/
static void sByteSwap(pUInt32 ioBuffer, int inLength)
{
...
/**
@brief Updaye MD5 hash value with new data

@param inData	Buffer to update hash with
@param inLength	Length of buffer in bytes
**/
void					MD5::Compute(pcVoid inData, int inLength)
{
	gAssertT(mState==OPEN, "Can't compute hash value before Open() has been called!");
	...
	{
		gMemCopy((pUInt8)mBuffer.mInput + buffer_offset, inData, inLength);
		return;
	}

	// Fill up current buffer until it's full
	pcUInt8 data   = (pcUInt8)inData;
	int     length = inLength;
	gMemCopy((pUInt8)mBuffer.mInput + buffer_offset, data, space_left);
	...
	while (length >= 64)
	{
		gMemCopy((pUInt64)mBuffer.mInput, (pUInt64)data, 64);
		sByteSwap(mBuffer.mInput, 16);
		Transform();
		data   += 64;
		length -= 64;
	...
	gMemCopy(mBuffer.mInput, data, length);
}
...
MD5Hash					MD5::Close()
{
	...
	if (mState == OPEN)
	{
		...
		pUInt8  p		= (pUInt8)mBuffer.mInput + count;
		...
		*p++ = 0x80;
		...
		mBuffer.mInput[14] = mLength << 3;
		mBuffer.mInput[15] = mLength >> 29;
...
**/
MD5Hash					gMD5Hash(pcVoid inBuffer, int inLength)
{
	MD5 md5;
	md5.Open();
