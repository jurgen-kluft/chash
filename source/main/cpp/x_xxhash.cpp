...
namespace xcore
{
	...
	struct xxhash64_ctxt
	{
	   ...
	   s32 m_memsize;
		...
		#define XXH_COPY_MEMORY(dst, src, length) { xbyte* pdst = (xbyte*)(dst); xbyte const* psrc = (xbyte const*)(src); for (s32 i=0; i<(length); ++i) { pdst[i] = psrc[i]; } }
		...
		void update(xcbuffer const& _buffer)
		{
			...
			if (m_memsize + len < 32) {  /* fill in tmp buffer */
				XXH_COPY_MEMORY(((xbyte*)m_mem64) + m_memsize, p, len);
				m_memsize += (u32)len;
				return;
			}

			if (m_memsize) {   /* tmp buffer is full */
			...
			if (p+32 <= bEnd) {
				...
				do {
					v1 = round(v1, read64bits(p)); p+=8;
					v2 = round(v2, read64bits(p)); p+=8;
					v3 = round(v3, read64bits(p)); p+=8;
					v4 = round(v4, read64bits(p)); p+=8;
				} while (p<=limit);
			...
			if (p < bEnd) {
				XXH_COPY_MEMORY(m_mem64, p, (size_t)(bEnd-p));
				m_memsize = (unsigned)(bEnd-p);
			}
		}

		u64 finalize(u64 h64, const void* ptr, size_t len)
		{
		...
		void digest (xbuffer& hash)
		{
			...
			h64 += (u64) m_total_len;
			u64 digest = finalize(h64, m_mem64, (size_t)m_total_len);

			hash.m_mutable[0] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[1] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[2] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[3] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[4] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[5] = (digest & 0xFF); digest = digest >> 8;
			hash.m_mutable[6] = (digest & 0xFF); digest = digest >> 8;
