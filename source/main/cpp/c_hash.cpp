...
namespace ncore
{
    static inline void ctxt_clear(hashtype_t ctxt, u32 size, u16 type)
    {
        u8* data = (u8*)ctxt;
    ...
    hashtype_t create_hash_ctxt(alloc_t* allocator, EHash type)
    {
        hashtype_t ctxt;
        switch (type.value)
        {
            case EHash::MD5:
                ctxt = allocator->allocate(sizeof(md5_t));
                ctxt_clear(ctxt, sizeof(md5_t), EHash::MD5);
                break;
            case EHash::SHA1:
                ctxt = allocator->allocate(sizeof(sha1_t));
                ctxt_clear(ctxt, sizeof(sha1_t), EHash::SHA1);
                break;
            case EHash::Skein256:
                ctxt = allocator->allocate(sizeof(skein256_t));
                ctxt_clear(ctxt, sizeof(skein256_t), EHash::Skein256);
                break;
            case EHash::Skein512:
                ctxt = allocator->allocate(sizeof(skein512_t));
                ctxt_clear(ctxt, sizeof(skein512_t), EHash::Skein512);
                break;
            case EHash::Skein1024:
                ctxt = allocator->allocate(sizeof(skein1024_t));
                ctxt_clear(ctxt, sizeof(skein1024_t), EHash::Skein1024);
                break;
            case EHash::Murmur32:
                ctxt = allocator->allocate(sizeof(murmur32_t));
                ctxt_clear(ctxt, sizeof(murmur32_t), EHash::Murmur32);
                break;
            case EHash::Murmur64:
                ctxt = allocator->allocate(sizeof(murmur64_t));
                ctxt_clear(ctxt, sizeof(murmur64_t), EHash::Murmur64);
                break;
            case EHash::XXHash64:
                ctxt = allocator->allocate(sizeof(xxhash64_t));
                ctxt_clear(ctxt, sizeof(xxhash64_t), EHash::XXHash64);
                break;
    ...
    void destroy_hash_ctxt(alloc_t* allocator, hashtype_t ctxt)
    {
    ...
    void hash_end(hashtype_t ctxt, u8* out_hash, s32 size)
    {
        hash_header_t* hash = (hash_header_t*)ctxt;
        switch (hash->type)
        {
