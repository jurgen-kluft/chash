...
UNITTEST_SUITE_BEGIN(crc)
{
    UNITTEST_FIXTURE(main)
    {
        ...
        UNITTEST_TEST(CRC_CRC32)
        {
            ...
            s64 len         = 128;
            CHECK_NOT_EQUAL(crc_t::crc32(cbuffer_t(buffer, buffer + len), 1321), crc_t::crc32(cbuffer_t(buffer, buffer + len), 654321));
            CHECK_NOT_EQUAL(crc_t::crc32(cbuffer_t(buffer, buffer+254), 110), crc_t::crc32(cbuffer_t(buffer, buffer+25), 110));
            CHECK_EQUAL(crc_t::crc32(cbuffer_t(buffer, buffer + len), 0), crc_t::crc32(cbuffer_t(buffer, buffer + len)));
            CHECK_NOT_EQUAL(crc_t::crc32(cbuffer_t(buffer, buffer + len), 1321), crc_t::crc32(cbuffer_t(buff2, buff2 + len), 654321));
        }
        UNITTEST_TEST(CRC_Adler16)
        {
            ...
            u8  buff2[512]  = {1, 1, 2, 3, 5, 8, 13, 21};
            s64 len         = 100;
            CHECK_EQUAL(crc_t::adler16(cbuffer_t(buffer, buffer + len), 1), crc_t::adler16(cbuffer_t(buffer, buffer + len)));
        ...
        UNITTEST_TEST(CRC_Adler32)
        {
            ...
            u8  buff2[512]  = {1, 1, 2, 3, 5, 8, 13, 21};
            s64 len         = 100;
            CHECK_EQUAL(crc_t::adler32(cbuffer_t(buffer, buffer + len), 1), crc_t::adler32(cbuffer_t(buffer, buffer + len)));
